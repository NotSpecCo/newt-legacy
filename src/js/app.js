...
var Newt = (function () {
	...
	function init() {
		...
		deleteTarget.addEventListener('drop', function (ev) {
			...
			if (ev.dataTransfer.types.includes('sitedivid')) {
				let element = this.ownerDocument.getElementById(
				...
				showConfirmPrompt(
					'Are you sure you want to delete this card? Sites in it will also be deleted.',
					'confirmDeleteCard',
					cardID
				);
			}
	...
	function changeTheme(theme) {
		...
		let selectedTheme = allThemes.find((theme) => {
				...
				for (var card of cards) {
					...
					ele.addEventListener('drop', function (ev) {
						if (!ev.dataTransfer.types.includes('sitedivid')) {
							...
							let fromCard = document.getElementById(
								'card' + ev.dataTransfer.getData('carddivid')
							);
							let toCard = this;
	...
	function changeTab(tab, direction) {
		...
		if (direction) {
			...
			if (direction === 'next') {
				tab =
					currentIndex + 1 > buttons.length - 2
						? buttons[0].action
						: buttons[currentIndex + 1].action;
			} else {
				tab =
					currentIndex - 1 < 0
						? buttons[buttons.length - 2].action
						: buttons[currentIndex - 1].action;
			}
		}
	...
	function saveCustomTheme() {
		...
		if (!abortSave) {
			if (EditingTheme) {
				let index = AppPrefs.customThemes.findIndex(function (x) {
					return x.id == EditingThemeID;
				});
	...
	function deleteTheme(theme) {
		var index = AppPrefs.customThemes
			.map(function (x) {
				return x.id;
			})
			.indexOf(theme);
